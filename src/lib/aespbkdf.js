...
export function createCipher(key, options = {}) {
  let { iv, alg } = options;
  if (!iv) iv = crypto.randomBytes(AES_IV_LEN);
  if (!alg) alg = DEFAULT_AES_ALG;
...
export function createDecipher(key, options) {
  ...
  if (!tag) throw new Error(`the authentication tag is required`);
  if (!iv) throw new Error("iv is a required option");
  if (!alg) alg = DEFAULT_AES_ALG;
...
export async function deriveAESKey(password, options = {}) {
  ...
  if (typeof iterations === "undefined") iterations = DEFAULT_ITERATIONS;

  if (typeof iterations === "number" && iterations < MIN_ITERATIONS)
    throw new Error(
      `${iterations} is less than ${MIN_ITERATIONS}, this isn't sensible or safe`
    );
