...
import { ObjectCodec, LeafObject, leafHash, directPreimage } from "./objects.js";
...
describe("LogicalTopology tests", function () {
  ...
  it("Should encode exitMenu proofs", function () {
    ...
    for (let i=0; i < proof.length; i++)
      expect(proof[i]).to.equal(proof1[i]);

    lo = new LeafObject({type:ObjectType.ExitMenu, leaf: new ExitMenu([[0], [1], [0], [0]])});
    ...
    prepared = [ObjectType.ExitMenu, directPreimage([[1, 0]])]; // side 1 (west), exit 0
    proof = trie.getProof(prepared);
    expect(proof.length).to.be.greaterThan(0);
  });
  ...
  it("Should encode location exit associations", function () {
    const topo = new LogicalTopology();
    topo.extendJoins([{ joins: [0, 1], sides: [3, 1] }]); // rooms 0,1 sides EAST, WEST
    topo.extendLocations([
      { sides: [[], [], [], [0]], flags: {} },
