...
export function hardhatKeyAliasAddress(alias) {
  if (!isHardhatAlias(alias)) return alias;
...
export function resolveHardhatKey(key) {
  ...
  const which = parseInt(parts[1]);
  if (which <= 0 || which >= wellknown.length)
    return wellknown[wellknown.length - 1];
  return wellknown[which];
}
